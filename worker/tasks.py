...
# 카운터 TTL은 '청소용 백스톱'일 뿐이다. 실제 수명은 진입 시 리셋 + 청산 시 삭제
# (_reset_position_state)가 관리한다. 과거 TTL 1h는 마지막 add 후 1시간이 지나면
# 카운터가 증발해 보유 중인 포지션에 무제한 추가매수를 허용하는 결함이었음
# (감사 재현: 단일 포지션 add 133회 → 증거금 소진까지 증식).
_PYRAMID_COUNT_TTL_SEC    = 86400  # 24h 백스톱 (RULE/ML 모드 포지션 수명 4h를 충분히 커버)
...
# 자본 방화벽: 단일 심볼 1회 진입 노셔널이 가용 마진의 이 비율 초과 시 주문 거부.
# ⚠️ 사이징의 '상한 백스톱'이므로 반드시 최대 진입 노셔널보다 커야 한다:
#    RISK_FACTOR × (변동성타겟 ON이면 vol_scale_max) < MAX_CAPITAL_PER_SYMBOL_PCT
# 과거 0.20 하드코딩이라 RISK_FACTOR를 0.20 넘게 올리면 모든 신규 진입이
# 조용히 거부되어 봇이 무거래 상태가 되는 함정이 있었다 → .env로 이동(기본 0.60).
MAX_CAPITAL_PER_SYMBOL_PCT = Decimal(str(settings.max_capital_per_symbol_pct))
if MAX_CAPITAL_PER_SYMBOL_PCT <= Decimal("0") or MAX_CAPITAL_PER_SYMBOL_PCT > Decimal("1"):
    logger.warning(
        "⚠️ [CAPITAL_GUARD] MAX_CAPITAL_PER_SYMBOL_PCT=%s 가 허용범위(0~1]를 벗어남 → 0.60으로 보정",
...
if _TREND_MODE:
    ...
    logger.warning(
        ...
        "타임아웃/하드TP/타이트SL/트레일링/불타기 OFF, 재난 스톱(-15%)만 유지. "
...
def _reset_position_state(symbol: str) -> None:
    """
    포지션 수명 종료/시작 시 파생 상태를 일괄 정리한다.
      - Peak ROI 레지스터 (트레일링 스탑 기준점)
      - 불타기 Redis 카운터
    이전 포지션의 잔류 값이 다음 포지션의 트레일링/불타기 판단을 오염시키는 것을
    방지한다. 진입(신규)·청산(손절방패/하드가드/역시그널) 모든 경로에서 호출할 것.
    """
    key = _normalize_symbol(symbol)
    _peak_roi_register.pop(key, None)
    try:
        if _r_sync is not None:
            _r_sync.delete(f"{_PYRAMID_COUNT_KEY_PREFIX}{key}")
    except Exception as exc:
        logger.warning(
            "⚠️ [포지션 상태 정리] 불타기 카운터 삭제 실패 (TTL %ds 백스톱 대기): %s",
            _PYRAMID_COUNT_TTL_SEC, exc,
        )
...
# 🛑 일일 최대손실 서킷브레이커
# ─────────────────────────────────────────────
# 브레이커가 만든 PAUSED를 다음 KST 날에 자동 해제하기 위한 날짜 마커.
# 수동 /pause는 이 마커를 남기지 않으므로 절대 자동 해제되지 않는다.
_BREAKER_PAUSE_DATE_KEY = "quantflow:breaker_pause_date"


def _check_daily_loss_breaker(current_equity: Decimal) -> bool:
    """
    당일(KST) 시작 자본 대비 손실이 settings.max_daily_loss_pct에 도달하면
    Redis sys_status=PAUSED + 날짜 마커를 기록하고 True를 반환한다.

    - True의 의미는 '신규 진입 동결'이다. 보유 포지션의 보호 가드(손절 방패·하드
      가드·역시그널 청산)는 호출부(analyze_and_trade)가 계속 작동시킨다.
    - 당일 시작 자본은 그날 첫 호출 시 Redis에 SETNX로 1회 기록(KST 날짜별 키, 2일 만료).
    - 한도(<=0)면 비활성으로 항상 False.
    - 발동 시 텔레그램 긴급 경고를 1회만 발송.
    - 다음 KST 날이 되면 _maybe_auto_resume_breaker()가 RUNNING으로 자동 복귀시킨다.
    """
    try:
    ...
    try:
        ...
        if daily_pnl <= -limit:
            ...
            _r_sync.set(_BREAKER_PAUSE_DATE_KEY, kst_date, ex=172800)
            if not already_paused:
                logger.error(
                    "🛑 [서킷브레이커] 일일 손실 한도 도달 → 신규 진입 동결(PAUSED): "
                ...
                notifier.send_message(
                    ...
                    f"• <b>조치:</b> <code>신규 진입 동결 — 보유 포지션의 손절/청산 가드는 계속 작동</code>\n"
                    f"• 다음 KST 자정에 자동 재개, 즉시 재개는 <code>/resume</code>\n"
                    f"━━━━━━━━━━━━━━━━━━━━"
                )
            return True
        return False
...
def _maybe_auto_resume_breaker(r) -> bool:
    """
    서킷브레이커가 만든 PAUSED를 다음 KST 날에 자동 해제한다.

    브레이커 발동 시 기록된 날짜 마커(_BREAKER_PAUSE_DATE_KEY)가 '오늘'이 아니면
    RUNNING으로 복귀 + 마커 삭제 + 텔레그램 통지. 마커가 없는 PAUSED(수동 /pause)는
    건드리지 않는다.

    Returns:
        True  — 방금 자동 해제함 (매매 재개 가능)
        False — 해제 안 함 (오늘 발동했거나 수동 일시정지)
    """
    try:
        marker = r.get(_BREAKER_PAUSE_DATE_KEY)
        if not marker:
            return False
        today_kst = datetime.now(timezone(timedelta(hours=9))).strftime("%Y%m%d")
        if str(marker) == today_kst:
            return False
        r.set("quantflow:sys_status", "RUNNING")
        r.delete(_BREAKER_PAUSE_DATE_KEY)
        logger.warning("▶️ [서킷브레이커] KST 거래일 변경 — 전일 브레이커 해제, 매매 자동 재개")
        notifier.send_message(
            "▶️ <b>[QuantFlow] 서킷브레이커 자동 해제</b>\n"
            "전일 일일 손실 한도로 동결됐던 매매가 새 거래일(KST) 시작과 함께 재개되었습니다."
        )
        return True
    except Exception as exc:
        logger.warning("⚠️ [서킷브레이커] 자동 해제 검사 실패 (동결 유지): %s", exc)
        return False
...
def _vol_target_scale(symbol: str = "BTC/USDT") -> Decimal:
    ...
    ⚠️ 채택 여부 근거(2026-06-12, scripts/risk_factor_scenarios.py — 진입 시 1회 샘플링
    이라는 이 구현 그대로 재현): IS Sharpe는 개선되나 OOS Sharpe 0.70→0.51로 악화되어
    'IS/OOS 동시 우위' 기준 미달 → 기본 OFF. (과거 "0.68→0.81 양 구간 개선"은 매봉
    무비용 리밸런싱 모델 기준이라 이 구현과 다른 메커니즘이었음)
    """
...
def fetch_market_data_task(self, symbol: str = "BTC/USDT"):
    ...
    try:
        ...
        # 과거에는 이 행을 그대로 INSERT하고 conflict 시 지표만 갱신해, DB의 1m 캔들이
        # '봉 시작 직후 수 초 스냅샷'(고가≈저가≈시가, 거래량≈0)으로 영구히 남았다.
        # 완성봉(봉 시작 + 60초 경과)만 통과시켜 지표·신호·저장 전부 확정 데이터로 계산한다.
        _now_ms = int(time.time() * 1000)
        df = df[df["timestamp_ms"] + 60_000 <= _now_ms].reset_index(drop=True)
        if len(df) == 0:
            logger.warning(f"⚠️ 완성 캔들 없음: {symbol}")
            return {"status": "empty", "symbol": symbol}
        ...
        # ── 최근 N개 완성봉 Bulk Upsert (OHLCV 포함) ─────────────────────────
        # conflict 시 OHLCV까지 갱신한다. 과거 미완성 스냅샷으로 저장된 행이 이 창에
        # 들어오면 확정 값으로 자가 치유되고, 워커가 1~2분 죽었다 살아나도 빠진 봉이
        # 다음 수집에서 채워진다. (limit=500 fetch이므로 창 내 봉은 항상 포함됨)
        _UPSERT_TAIL_N = 5
        tail = df.iloc[-_UPSERT_TAIL_N:]
        values_list = []
        for _, row in tail.iterrows():
            values_list.append({
                "timestamp":   _ensure_utc(int(row["timestamp_ms"])),
                "symbol":      symbol,
                ...
                "sma_20":      _safe_dec(row, "sma_20"),
                "rsi_14":      _safe_dec(row, "rsi_14"),
                "bb_upper":    _safe_dec(row, "bb_upper"),
                "bb_lower":    _safe_dec(row, "bb_lower"),
                "atr_14":      _safe_dec(row, "atr_14"),      # [Phase 6] Numpy ATR
                "macd_line":   _safe_dec(row, "macd_line"),
                "macd_signal": _safe_dec(row, "macd_signal"),
                "macd_hist":   _safe_dec(row, "macd_hist"),
            })

        insert_stmt = pg_insert(MarketData).values(values_list)
...
def analyze_and_trade(self, symbol: str = "BTC/USDT"):  # noqa: C901
    ...
    try:
        ...
        # ── [원격 제어] /pause·서킷브레이커 동결 가드 ─────────────────────────
        # PAUSED의 의미 = '신규 진입/불타기 동결'. 보유 포지션의 보호 가드(손절 방패·
        # 하드 가드·역시그널 청산)는 계속 작동한다. (과거: PAUSED면 여기서 즉시 return
        # → 열려 있는 포지션의 손절·청산까지 정지되는 결함이 있었음)
        # 브레이커발 PAUSED는 KST 날짜가 바뀌면 _maybe_auto_resume_breaker()가 자동 해제.
        # Redis 조회 실패 시에는 가용성 우선으로 매매를 계속한다(기존 동작 유지).
        _trading_frozen = False
        try:
            ...
            if _sys_status is not None and str(_sys_status).upper() == "PAUSED":
                if _maybe_auto_resume_breaker(_status_r):
                    logger.info("▶️ [원격 제어] 브레이커 자동 해제 완료 — 매매 정상 진행")
                else:
                    _trading_frozen = True
                    logger.warning(
                        "⏸️ [원격 제어] 시스템 PAUSED — 신규 진입 동결 "
                        "(보유 포지션 보호 가드는 계속 작동)"
                    )
        ...
        # ── [서킷브레이커] 일일 최대손실 도달 시 신규 진입 동결 ─────────────────
        # 당일(KST) 시작 자본 대비 손실이 한도 초과면 sys_status=PAUSED로 전환.
        # 여기서 return하지 않는다 — 보유 포지션의 보호 가드는 아래에서 계속 작동해야
        # 하므로 동결 플래그만 세운다. (한도 미설정 시 비활성 — 기존 동작 영향 없음)
        ...
        for session in _get_sync_session():
            ...
            # ── [동결 게이트] PAUSED/브레이커 + 무포지션 → 여기서 종료 ──────────
            # 포지션 보유 중이면 계속 진행해 아래 보호 가드(손절 방패·하드 가드·
            # 역시그널 청산)를 작동시키고, 신규 진입·불타기만 차단된다
            # (진입은 FLAT에서만 발생하므로 이 게이트가 차단을 보장).
            if _trading_frozen and current_position == "FLAT":
            ...
            # 실제 체결(FILLED/PARTIALLY_FILLED)만 인정 — REJECTED/FAILED/CANCELLED
            # 기록이 폴백 가격으로 저장되어 유효한 진입을 막던 결함 수정.
            cooldown_cutoff = now_utc - timedelta(minutes=COOLDOWN_MINUTES)
            recent_same_side = session.query(TradeHistory).filter(
                TradeHistory.symbol == symbol,
                TradeHistory.side == action,
                TradeHistory.timestamp >= cooldown_cutoff,
                TradeHistory.status.in_(("FILLED", "PARTIALLY_FILLED")),
            ).order_by(desc(TradeHistory.timestamp)).first()

            # 단가 버퍼 가드는 '같은 방향 추가 진입(피라미딩)'의 평단 방어 장치다.
            # TREND 모드의 FLAT 신규 진입(스톱앤리버스 2번째 다리: 청산 직후 반대 진입)은
            # 방어할 평단이 없는데도 직전 청산 체결(같은 side)에 걸려 진입이 쿨다운
            # 만료까지 ~5분 지연되던 결함 → FLAT이면 가드 면제.
            _skip_buffer_guard = _TREND_MODE and current_position == "FLAT"

            if recent_same_side is not None and not _is_reverse_exit and not _skip_buffer_guard:
            ...
            #
            # ⚠️ TREND 모드에서는 전면 비활성: 검증된 추세전략(portfolio_backtest,
            # OOS Sharpe 0.78)에는 피라미딩이 없으며, IS/OOS 분리 검증
            # (scripts/audit_trend_fidelity.py --split-only)에서 피라미딩은 학습구간만
            # 좋고 검증구간에서 수익·MDD·Sharpe·PF 전 지표 열위(과최적)로 기각됐다.
            # TREND는 신호가 항상 BUY/SELL이라 수익 중 LONG에 매 사이클 이 분기가
            # 발화하므로, 막지 않으면 사이징 규율(RISK_FACTOR·변동성타겟)이 무너진다.
            _pyramid_executed = False
            if (
                (not _TREND_MODE)
                and (not _trading_frozen)   # PAUSED/브레이커 중 추가 매수 금지 (청산만 허용)
                and action == "BUY"
            ...
            if action == "SELL":
                if current_position == "LONG":
                    ...
                    # 신규 숏 진입 — 이전 포지션의 Peak ROI/불타기 카운터 잔류 값 정리
                    _reset_position_state(symbol)
                    logger.info(
                        "🔄 [신규 숏 진입] 포지션 파생 상태 초기화: symbol=%s (key=%s)",
                ...
                if current_position == "SHORT":
                    ...
                    # 신규 LONG 진입 — 이전 포지션의 Peak ROI/불타기 카운터 잔류 값 정리
                    _reset_position_state(symbol)
                    logger.info(
                        "🔄 [신규 롱 진입] 포지션 파생 상태 초기화: symbol=%s (key=%s)",
                        symbol, _normalize_symbol(symbol),
                    )
            ...
            # 역시그널 청산으로 포지션이 닫힌 경우 — Peak ROI/불타기 카운터 잔류 정리
            if trigger_type in ("REVERSE_SWITCH_EXIT_LONG", "REVERSE_SWITCH_EXIT_SHORT"):
                _reset_position_state(symbol)
...
def handle_telegram_command(command: str) -> str:
    ...
    if "/pause" in normalized_cmd:
        ...
        # 수동 일시정지는 브레이커 자동 해제 대상이 아님 — 날짜 마커 제거
        try:
            r.delete(_BREAKER_PAUSE_DATE_KEY)
        except Exception:
            pass
        ...
        _send_reply(
            "⏸️ [QuantFlow] 시스템이 일시 정지되었습니다. 신규 진입·불타기가 동결됩니다.\n"
            "보유 포지션의 보호 가드(손절 방패·재난 스톱·역시그널 청산)는 계속 작동하며, "
            "/resume 전까지 자동 재개되지 않습니다."
        )
        return "PAUSED"
