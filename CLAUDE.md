...
현재 전략: **4h EMA(30/60) 추세추종(`PREDICTOR_TYPE=TREND`) + 변동성 타게팅(`VOL_TARGET_ENABLED`)**.
...
- **전략 토글(.env)**: `PREDICTOR_TYPE=TREND`, `RISK_FACTOR`(사이징, 기본 0.10), `VOL_TARGET_ENABLED=true`, `MAX_DAILY_LOSS_PCT`(서킷브레이커, 0=off).
- **TREND 워밍업**: 4h EMA(30/60)은 4h봉 ≥62개(≈10일 연속 1m) 필요. 부족하면 HOLD. `scripts/backfill_db.py`로 과거 1m 백필 가능.
